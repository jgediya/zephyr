...
.. code-block:: devicetree
    ...
    &pinctrl {
        uart0_default: uart0_default {
            pins1 {
                pins = <NRF_PIN(0, 5, UART_TX);
            };
            pins2 {
                pins = <NRF_PIN(1, 1, UART_RX)>;
                bias-pull-up;
            };
        };
    ...
    for i, group in enumerate(groups):
        ...
        for i, mapping in enumerate(group.pins):
            ...
            if port == 1:
                pin -= 32
            content += (
                f"\t\t\t{prefix}<NRF_PIN({port}, {pin}, {mapping.signal})>{suffix}\n"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--header", type=str, help="Header to be prepended to pinctrl files"
    )
