...
/**
 * @brief Utility macro to build nRF pinctrl configuration for a pin.
 *
 * This macro shall be used to build pin configurations at runtime.
 *
 * @param port Port (0 or 1).
 * @param pin Pin (0..31).
 * @param fun Pin function configuration, e.g. UART_TX.
 * @param pull Pin pull configuration, e.g. PULL_UP.
 * @param drive Pin drive mode, e.g. DRIVE_S0S1.
 * @param lp Pin low power mode flag, e.g. LP_DISABLE.
 */
#define NRF_PINCFG(port, pin, fun, pull, drive, lp)			       \
	((((((port) * 32U) + (pin)) & NRF_PIN_MSK) << NRF_PIN_POS) |	       \
	 ((NRF_FUN_ ## fun & NRF_FUN_MSK) << NRF_FUN_POS) |		       \
	 ((NRF_ ## pull & NRF_PULL_MSK) << NRF_PULL_POS) |		       \
	 ((NRF_ ## drive & NRF_DRIVE_MSK) << NRF_DRIVE_POS) |	       \
	 ((NRF_ ## lp & NRF_LP_MSK) << NRF_LP_POS))
